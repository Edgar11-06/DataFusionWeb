...
{
    public class PostgresDataStore : IInMemoryDataStore
    {
        private readonly ConnectionStringProvider _provider;
        private readonly string _databaseName;

        public PostgresDataStore(ConnectionStringProvider provider, string databaseName = "datafusiondb")
        {
            _provider = provider ?? throw new ArgumentNullException(nameof(provider));
            _databaseName = string.IsNullOrWhiteSpace(databaseName) ? "datafusiondb" : databaseName;
        }

        private PostgresRepository CreateRepo()
        {
            if (!_provider.IsSet) throw new InvalidOperationException("La cadena de conexión no está establecida.");
            return new PostgresRepository(_provider.ConnectionString!, _databaseName);
        }
